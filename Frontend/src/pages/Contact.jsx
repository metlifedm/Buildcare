...
export default function Contact() {
  return (
    <>
      ...
      {/* Contact Info Cards */}
      <section className="py-20 bg-dark-900" aria-label="Contact information">
        <div className="container-custom">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {contactInfo.map((info, index) => (
              ...
              >
                <Card className="h-full text-center group" padding="lg" variant="gradient">
                  <div className={`w-14 h-14 rounded-2xl bg-dark-700/50 flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300`}>
                  ...
                  {info.details.map((detail, i) => (
                    <p key={i} className="text-dark-300 text-sm">{detail}</p>
                  ))}
                  {info.action && (
                    <a
                      href={info.action}
                      ...
                      className="inline-flex items-center gap-1.5 text-primary-400 text-sm font-medium mt-4 hover:gap-2.5 transition-all"
                    >
                      {info.actionLabel}
      ...
      {/* Contact Form + Map */}
      <section className="py-24 bg-dark-950" aria-label="Contact form">
        <div className="container-custom">
          <SectionHeading
          ...
          <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-12 max-w-6xl mx-auto">
            ...
            >
              {/* WhatsApp CTA */}
              <Card padding="lg" className="bg-gradient-to-br from-green-900/20 to-green-800/10 border-green-500/20">
                <div className="flex items-center gap-4 mb-4">
                  <div className="w-12 h-12 rounded-full bg-green-500/20 flex items-center justify-center">
                    <MessageCircle className="w-6 h-6 text-green-400" fill="currentColor" />
                  </div>
                  <div>
                    <h3 className="font-heading text-lg font-semibold text-dark-50">
                      WhatsApp Us
                    </h3>
                    <p className="text-green-400/70 text-sm">Instant Response</p>
                  </div>
                </div>
              ...
              <Card padding="none" className="overflow-hidden">
                ...
                <div className="p-4">
                  <a
                    ...
                    className="flex items-center justify-center gap-2 text-primary-400 text-sm font-medium hover:text-primary-300 transition-colors"
                  >
              ...
              <Card padding="lg" variant="gradient">
                <h3 className="font-heading text-lg font-semibold text-dark-50 mb-4">
                  Follow Us
                </h3>
                <p className="text-dark-300 text-sm mb-4">
                ...
                <div className="flex items-center gap-3">
                  {socialLinks.map((social) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-11 h-11 rounded-xl glass-light flex items-center justify-center text-dark-300 hover:text-primary-400 hover:border-primary-400/30 hover:-translate-y-1 transition-all duration-300"
      ...
      <section className="py-16 bg-dark-900 border-t border-dark-700/30" aria-label="Quick contact">
        <div className="container-custom">
          <div className="grid sm:grid-cols-3 gap-8 text-center">
            <motion.a
              href={`tel:${COMPANY.phone}`}
              className="group flex flex-col items-center gap-3 p-6 rounded-2xl hover:bg-dark-800/50 transition-all"
              whileHover={{ y: -5 }}
            >
              ...
              <div>
                <p className="text-dark-400 text-sm">Call Directly</p>
                <p className="text-dark-50 font-semibold text-lg">{COMPANY.phone}</p>
              </div>
            </motion.a>
            ...
            <motion.a
              ...
              whileHover={{ y: -5 }}
            >
              <div className="w-16 h-16 rounded-2xl bg-red-500/10 flex items-center justify-center group-hover:bg-red-500/20 transition-colors">
                <Mail className="w-8 h-8 text-red-400" />
              </div>
              <div>
                <p className="text-dark-400 text-sm">Email Us</p>
                <p className="text-dark-50 font-semibold text-lg">{COMPANY.email}</p>
              </div>
            </motion.a>

            <motion.button
              onClick={() => openWhatsApp()}
              className="group flex flex-col items-center gap-3 p-6 rounded-2xl hover:bg-dark-800/50 transition-all cursor-pointer"
              whileHover={{ y: -5 }}
            >
              <div className="w-16 h-16 rounded-2xl bg-green-500/10 flex items-center justify-center group-hover:bg-green-500/20 transition-colors">
                <MessageCircle className="w-8 h-8 text-green-400" fill="currentColor" />
              </div>
              <div>
                <p className="text-dark-400 text-sm">WhatsApp</p>
                <p className="text-dark-50 font-semibold text-lg">Instant Chat</p>
              </div>
            </motion.button>
          </div>
        </div>
